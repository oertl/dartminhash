...
double intersection(const std::vector<std::pair<uint64_t, double>>& x, const std::vector<std::pair<uint64_t, double>>& y) {
	uint64_t i = 0;
	uint64_t j = 0;
	double s = 0;
	while(i < x.size() && j < y.size()) {
		if(x[i].first == y[j].first) {
			s += std::min(x[i].second, y[j].second);
			i++;
			j++;
...
double jaccard_similarity(const std::vector<std::pair<uint64_t, double>>& x, const std::vector<std::pair<uint64_t, double>>& y) {
...
double l1_similarity(const std::vector<std::pair<uint64_t, double>>& x, const std::vector<std::pair<uint64_t, double>>& y) {
	double s = intersection(x, y);
	double w_x = weight(x);
	double w_y = weight(y);
	return s/std::min(w_x, w_y);
}

double hamming_distance(const std::vector<bool>& x, const std::vector<bool>& y) {
	double h = 0;
...
double onebit_minhash_jaccard_estimate(const std::vector<bool>& x, const std::vector<bool>& y) {
	double h = hamming_distance(x, y);
	double t = x.size();
	return std::max(0.0, 2*(1 - h/t) - 1);
}
...
double jaccard_similarity_from_l1_similarity(double x_weight, double y_weight, double l1_sim) {
  double i = std::min(x_weight, y_weight)*l1_sim;
...
double l1_similarity_from_jaccard_similarity(double x_weight, double y_weight, double jaccard_sim) {
  ...
  return i/std::min(x_weight, y_weight);
}

// Count the number of collisions in two vectors of minhash sketches ((id, rank) pairs)
uint64_t count_collisions(const std::vector<std::pair<uint64_t, double>>& x, const std::vector<std::pair<uint64_t, double>>& y) {
